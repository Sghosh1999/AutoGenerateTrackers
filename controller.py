...
def main():
    ...
    def generate_weekly_trackers():
        ...
        try:
            uploaded_file = col2.file_uploader("Upload the Daily Trcakers as serially as Names",type=['csv', 'xlsx', 'xlsm'],accept_multiple_files=False)
            uploaded_data_read = pd.read_excel(uploaded_file,engine='openpyxl')
            week_tracker = pd.DataFrame(obj1.weekly_tracker_generator(uploaded_data_read, [user_input]))
            month_tracker = pd.DataFrame(obj1.monthly_tracker_generator(uploaded_data_read, [user_input]))
    ...
    def generate_weekly_trackers_multiuser():
        ...
        uploaded_files = st.file_uploader("Upload the Daily Trcakers as serially as Names",type=['csv', 'xlsx', 'xlsm'],accept_multiple_files=True)
        ...
        if uploaded_files:
            ...
            for file,usenames in zip(uploaded_files,user_input.split(',')):
                ...
                weekly_tracker = pd.DataFrame(obj1.weekly_tracker_generator(uploaded_data_read, [usenames]))
                monthly_tracker = pd.DataFrame(obj1.monthly_tracker_generator(uploaded_data_read, [usenames]))
